...
namespace CaseStudyKampusLearnAPI.Controllers
{
	...
	public class CourseController : ControllerBase
	{
		...
		public IActionResult GetCourse()
		{
			...
			//				   join admins in adminlist
			//				   on courses.AdminId equals admins.AdminId
			//				   into coursetrainerjoin
			//				   from admin in coursetrainerjoin.DefaultIfEmpty()
			//				   select new { courses, admin };

			logger.LogInformation("Course details are listed");
			return StatusCode(200, course.Where(status => status.IsActive == true));
		}
		...
		public IActionResult GetCourseByCategory(string courseCategory)
		{
            ...
            {
                var courseList = course.Where(o => o.CourseCategory.ToLower().Contains(courseCategory.Trim().ToLower()) );
				if (courseList.Count()== 0)
				{
					return StatusCode(200, $"No course with {courseCategory}");
				}
				else
				{
					return StatusCode(200, courseList);
				}
            }
