...
namespace CaseStudyKampusLearnAPI.Controllers
{
	...
	public class TrainerController : ControllerBase
	{
		...
		public IActionResult getTrainer()
		{
			try
			{
				...
				if (trainer.Count != 0)
				{
					...
					return StatusCode(200, trainer.Where(status => status.IsActive == true));
				}
				else
				{
		...
		//Get trainer by search
		[HttpGet("search/{name}")]
		public IActionResult GetTrainersbySearch(string name)
		{
			try
			{
				List<Trainer> trainers = repo.Trainer.ToList();
                List<Admin> adminlist = repo.Admin.ToList();
				if (trainers.Count == 0)
				{
					return StatusCode(200,$"No trainers with { name}" );
				}
                if (name == "...")
				{
					...
					var trainersList = trainers.Where(o => o.Name.ToLower().Contains(name.Trim().ToLower()) || o.Qualification.ToLower().Contains(name.Trim().ToLower()));
					if (trainersList.Count() == 0)
					{
						return StatusCode(200, $"No trainer with {name}");
					}
					else
					{
						return StatusCode(200, trainersList);
					}
				}
            }
            catch (NullReferenceException ex)
