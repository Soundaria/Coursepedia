...
namespace CaseStudyKampusLearnAPI.Controllers
{
	...
	public class CandidateController : ControllerBase
	{
        ...
        [HttpGet("search/{name}")]
        public IActionResult GetCandidatebySearch(string name)
        {
            try
            {
                List<Candidate> candidate = repo.Candidate.ToList();
                ...
                {
						var candidateList = candidate.Where(o => o.Name.ToLower().Contains(name.Trim().ToLower()));
                    return StatusCode(200, candidateList);
                }
            }
            catch (NullReferenceException ex)
            {
		...
		public IActionResult AddCandidate([FromBody] Candidate candidate)
		{
			try
			{
                ...
                var duplicateUser = userLists.Find(x => x.Email == candidate.Email);
				if (duplicateUser == null)
				{
					...
					repo.User.Add(user);
					repo.SaveChanges();
					List<User> users = repo.User.ToList();
					var id = users.Find(x => x.Email==candidate.Email);
					candidate.CandidateId = id.UserId;
