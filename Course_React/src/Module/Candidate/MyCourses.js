...
export default function MyCourses(){
   ...
   // const colours = ['burlywood','bisque','cadteblue','darkcyan','darkgoldenrod','darkolivegreen','darkslategrey','lightblue','lavendar','lightgreen','tan','thistle'];
    ...
    const getMyCourse=()=>{
      axios.get(`https://localhost:44307/api/CandidateCourse/GetCourseOfCandidate/${JSON.parse(localStorage.getItem("CandidateId"))}`
      ).then((response)=>{
   ...
   function PDF(){
    ...
    doc.setFontSize(20);
    doc.text("Coursepedia Bill Receipt",70,20);
    autoTable(doc,{
        margin: { top: 27 },
    ...
    const pay=(e)=>{
        //setDisable(true);
        axios.post(`https://localhost:44307/api/Payment/AddPayment/${JSON.parse(localStorage.getItem("CandidateId"))}`,{
            "courseId":e
        }).then((response)=>{
            ...
            mycourse.map((data)=>{
                return(
                    <Grid.Col span={3} className={styles.Card}>
                        <MyCourseCard desc={()=>{descFunc(data.course.description)}} badge={data.status} duration={data.course.durationInHours}day={data.course.weekday}  time={data.course.time}    title={data.course.courseName}>
                         {  
                            data.status==="Active"?
                            <>
                            <br></br>
